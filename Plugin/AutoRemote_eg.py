...
For more information about AutoRemote, see: http://joaoapps.com/autoremote/
...
eg.RegisterPlugin(
    ...
    canMultiLoad=True,
    icon=("iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAABmJLR0QA/wD/AP+gvae"
          "TAAAACXBIWXMAAA7EAAAOxAGVKw4bAAAAB3RJTUUH3AgXChElQdwgkgAAA2NJREFUOM"
          "uFkktsVGUYht9zm3Pm2pkOzPQylKGFTsWCNCrBUCuYtEhjrVQ04oY00WjcuSExISEKJ"
          "izcGOKaeKlau6jBpAEheCm9oNQ2LdbpWNopQy8znUudc5n/nDnn/C5MSVnx7Z/nTd73"
          "Y/CYuzg+CZgmt564R6WaWubCsVarvfdCJJ7M9D4ZEr/kHyewJY/AWNY5YVs4UZbc0Q9"
          "/ntXdseZn6w/t7Vamf2EYALj52y282Nb6CHjt1xEce+EweoqUb8nLqeL6xlqR8kE1b/"
          "B6kcxWmWrqCT85z2wClFLcnpqpUxXVo+Syie5Xu8zRiclWT2Uw00+kd6cX1RPbYed11"
          "azJZWSDplNnr3184mtuUxBrfnpnciVzKV2QT9ngbg4OfGP8eH1kaD2TOyK5g/0JTeow"
          "/lVlLVtaVu4vO4r3k/Hw7tY7DzugHGvplrWtbOI5j8c50H/llmmYbKNByo2t09/e9dq"
          "S67PcYWcpueSSSH6OKhsLllak/NjY+A5eEusfZOQGr8UdpGAgOvgW2ynCtbSGyql+qm"
          "jZk02nz3/RwSkTN+4VatOpB20BobTHJfIM89XAlcHVvPKKz+uzwDFCVTiECiKDzAxil"
          "z4Jj9+PO0I7XfU2MLKs3Ihsr3jnrZ5LqX0v7WdcDpj86npBkInFuj0VrBcGaue+RzX3"
          "D8JtzZBnAxh19iDPR5lCdg12SYUg+CgwZM1cHQIAsGfeP/1yY/2Oy5wggJNzqJDnEdx"
          "dB1vfAFlRwaXmoZY1RAKe4bMfvN3+elfn0uXvBh/Ozf9w9XrDhma5wXBAaQOWJoPRCT"
          "iXD5peABvbA6pZ0B28c/jPeGR5cW71zde6rU0Bm0ytfF6UtTcI0eELB2gwVgdZNrGWA"
          "ZTYSawbTvA8D4WQZ6Zn5/sUQuq2PhyvaiRtEboQY/JGZbyvKe6W6IJrH0P8UdDKKC2z"
          "NlyClcrn5WUNkA/s3VV+RLAzEj7j11Z6henhcznf/lIq3OIk3mqwLAvWKjNOgSOhgKe"
          "zkE2rokPkOo8+v7xVwABAZuqnpsTCyntjWfGUCTYUjVR95PeIZHzir08Eh2g/1Vh7pK"
          "v96Mgm9PvdBA42N/7fweLffyB0oCO+SIOf6oY+KjL2mAPlvuNthy46WDrjYO3Jmupwe"
...
def download_file(file_url, folder):
    ...
    'file_url' if the file isn't online (ie file_url doesn't start with "http"),
    ...
    if file_url.startswith("http"):
        ...
        meta = u.info()
        content_disposition = meta.getheaders('Content-Disposition')
        if len(content_disposition) > 0:
            file_name_from_server = get_file_name_from_server(content_disposition[0])
            if file_name_from_server is not None:
    ...
    if plugin.localIp == "":
        ...
        if not success:
            ...
            if not local_ip.startswith("127"):
                pass
                # print "Normal " + local_ip

        # print "Current local IP detected: " + local_ip
    else:
        local_ip = plugin.localIp
    return local_ip


def on_browse_file(event, ctrl, options=None):
...
def get_event_ghost_exe_path():
    parent_folder = parent_dir(parent_dir(parent_dir(os.path.realpath(__file__))))
    return parent_folder + "\\EventGhost.exe"


def define_action_on(file_type, registry_title, command, title=None):

    try:
        # print("Creating " + title + " action in the Windows Context Menu "
        #       "(command: " + command + ")")
        """
        define_action_on(file_type, registry_title, command, title=None)
            file_type: either an extension type (ex. ".txt") or one of the
            ...
            registry_title: the title of the sub-key, not important, but
        ...
        k1 = _winreg.CreateKey(reg, file_type)
...
def create_send_to_shortcut(device):
    """Create a "Send to" shortcut in the Windows Start menu"""

    send_to_folder = (os.environ.get('USERPROFILE') +
                    '\\AppData\\Roaming\\Microsoft\\Windows\\SendTo\\')
    path = send_to_folder + device.name.encode('utf-8') + ".lnk"
    target = get_event_ghost_exe_path()
    w_dir = parent_dir(get_event_ghost_exe_path())
    icon = get_event_ghost_exe_path()
...
def first(a_list):
    ...
    try:
        return next(iter(a_list))
    except:
        return


def device_by_key(plugin, key):
...
def get_file_name_from_server(content_disposition):
    if len(content_disposition) > 0:
        file_name_from_server = FILENAME_FROM_DL_PATTERN \
                                 .search(content_disposition) \
                                 .groups(1)
        if file_name_from_server is not None:
            return file_name_from_server[0]
    return None


def get_savable_devices(devices):
    return [(i.name, i.url, i.key, i.localIp, i.tryLocalIp, i.port)
            for i in devices]


def save_config(plugin):
    tr_item = plugin.info.treeItem
    args = list(tr_item.GetArguments())

    savable_devices = get_savable_devices(plugin.devices)
    args[3] = savable_devices
...
class FileLoader(jinja2.BaseLoader):
    ...
    def get_source(self, environment, filename):
        ...
        try:
            contents = source_file.read().decode("utf-8")
        finally:
            source_file.close()
...
class AutoRemotePayload(object):
    ...
    def get_description(self):
        ...
        if self.plugin is not None:
            do_all_logs = self.plugin.systemLogs
        if do_all_logs:
            result = ""
...
def get_communication_from_content(content, egClass):
    request = content
    ...
    # request_json_string = unquote_plus(request)
    request_json = json.loads(request_json_string)
    comm_type = request_json.get("communication_base_params").get("type")
    print "Got communication of type: " + comm_type
    communication = eval(comm_type + "(egClass)")
    communication.FromJson(request_json)
...
class MyHTTPRequestHandler(SimpleHTTPRequestHandler):
    ...
    def Authenticate(self):
        ...
        if auth_header is not None:
            ...
            if auth_type.lower() == 'basic' and auth_string == self.authString:
                return True
    ...
    def do_PUT(self):
        if (self.plugin.fileFolder == "" or
            ...
            fileNameFromServer = get_file_name_from_server(
    ...
    def do_POST(self):
        ...
        communication = get_communication_from_content(content, self)
...
class GoogleDrive(object):
    ...
    def InsertPermission(self, fileId, value, kind, role):
        url = ("https://www.googleapis.com/drive/v2/files/" + fileId +
               "/permissions")
        params = {"role": role, "type": kind, "value": value}
...
class Communication(object):
                        ...
                        with io.FileIO(aFile, 'r') as content_file:
                            ...
                            if isLocalRequest:
                                try:
                                    ...
                                    response = f.read()
                                    comm = get_communication_from_content(
                                        response,
                                        egClass)
    ...
    def SendSync(self, egClass):
        device = device_by_key(egClass.plugin, self.key)
        if device is not None:
            myLocalIp = get_local_ip(egClass.plugin)
            ...
            if f is not None:
                response = f.read()
                if isLocalRequest:
                    communication = get_communication_from_content(
                        response,
                        egClass)
...
class Message(Request):
    ...
    def downloadFile(self, fileUrl, folder):
        return download_file(fileUrl, folder)
    ...
    def executeRequest(self, egClass):
        ...
        payload = AutoRemotePayload(
            ...
            device_by_key(plugin, self.sender), plugin, self)
...
class SendMessage(eg.ActionBase):
    ...
    def OnBrowseFiles(self, event):
        on_browse_file(event, self.filesCtrl, wx.FD_MULTIPLE)


def registerDevice(plugin, deviceToRegister):
    try:
        device = device_by_key(plugin, deviceToRegister.id)
...
class RequestGetRegistration(Request):
    ...
    def executeRequest(self, egClass):
        plugin = egClass.plugin
        localip = get_local_ip(egClass.plugin)
...
class RegisterEventGhost(eg.ActionBase):
    ...
    def __call__(self, name="", url="", key="", ttl=""):
        ...
        localip = get_local_ip(self.plugin)
...
class SendNotification(eg.ActionBase):
    ...
    def OnBrowseFileIcon(self, event):
        on_browse_file(event, self.iconCtrl)

    def OnBrowseFilePicture(self, event):
        on_browse_file(event, self.pictureCtrl)
...
class AutoRemote(eg.PluginBase):
    ...
    def __start__(self, prefix=None, port=1818, dname="EventGhost", devices=[],
        ...
        define_action_on(
            "*", "SendToEventGhost",
            get_event_ghost_exe_path() + " -event SentFromExplorer.File \"%1\"",
    ...
    def Configure(self, prefix="HTTP", port=1818, name="EventGhost",
        ...
        while panel.Affirmed():
            panel.SetResult(
                ...
                portCtrl.GetValue(),
                nameCtrl.GetValue(),
                get_savable_devices(self.cdevices),
